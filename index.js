...
var root = {
  ...
  deleterestaurant: ({ id }) => {
    ...
    const ok = restaurants.findIndex(r => r.id == id) >= 0;
  ...
  editrestaurant: ({ id, name }) => {
    ...
    var iRestaurant = restaurants.findIndex(r => r.id == id);

    if (iRestaurant < 0) {
      throw new Error("restaurant doesn't exist");
    }

    restaurants[iRestaurant].name = name;

    return restaurants[iRestaurant];
  },
};
var app = express();
app.use(
